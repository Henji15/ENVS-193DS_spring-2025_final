...
\[In part 1, we found a (insert effect size) relationship between distance from headwater (km) and annual total nitrogen load (kg year⁻) (pearson's correlation test, r = correlaton coefficient, p = 0.03, $\alpha$ = significance level). For part 2, we found a difference ($\eta$\^2 = effect size) between urban land, atmospheric deposition, fertilizer, wastewater treatment, and grassland in average nitrogen load (kg year⁻) (one-way ANOVA , F(among groups df, within groups df) = test statistic, p = 0.02, $\alpha$ = significance level). On average, urban land showed (more, less...) nitrogen load (kg year⁻¹) (__ kg year⁻, % confidence interval: \[lower bound, upper bound\] kg year⁻) than atmospheric deposition (this can expanded for more variables).\]
