...
```{r}
predicted_vals <- ggpredict(model1, terms = c("edge_distance [all]", "season")) |>
  rename(season = group) 

nest_data <- nest_boxes_clean |> 
  mutate(
    sp = as.numeric(sp),
...
ggplot(data = nest_data, 
  ...
  geom_line(data = predicted_vals,
            aes(x = x, y = predicted, color = season),
            linewidth = 1.1, inherit.aes = FALSE) +

  scale_y_continuous(limits = c(0, 1), breaks = c(0, 1, 0.5)) +
  scale_fill_manual(values = c("2016" = "#d95f0e", "2019" = "#1b9e77")) +
  scale_color_manual(values = c("2016" = "#d95f0e", "2019" = "#1b9e77")) +
  ...
  labs(
  title = "Swift Parrot Nest Box Use based on Distance from Forest Edge",
